...
def get_neptune_temp(band: str, ufm: str) -> float:
    """
    Get the temperature of Neptune for a given band.
    ...
    -------
    neptune_temp : float
        The temperature of Neptune in Kelvin.
    ...
    """
    hdu = fits.open(
        "/global/u2/j/jorlo/dev/LAT_Loading/latcom/planet_models/neptune_esa5.fits"
    )
    data = hdu[1].data
    freqs = data["wave"]
    T_rj = data["T_rj"]
    ...
    if band == "030":
        neptune_temp = interp(27.9)
    elif band == "040":
        neptune_temp = interp(40.2)
    elif band == "090" or band == "150" or band == "220" or band == "280":
        neptune_temp = interp(bandcenter)
    else:
