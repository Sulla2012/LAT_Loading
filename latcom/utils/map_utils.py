...
sim_list = {
    "baseline": {
        "LF": os.path.join(
            os.environ["JBOLO_MODELS_PATH"],
            "V3r8/V3r8_Baseline/LAT/V3r8_Baseline_LAT_LF.yaml",
        ),
        "MF": os.path.join(
            os.environ["JBOLO_MODELS_PATH"],
            "V3r8/V3r8_Baseline/LAT/V3r8_Baseline_LAT_MF.yaml",
        ),
        "UHF": os.path.join(
            os.environ["JBOLO_MODELS_PATH"],
            "V3r8/V3r8_Baseline/LAT/V3r8_Baseline_LAT_UHF.yaml",
        ),
    },
    "goal": {
        "LF": os.path.join(
            os.environ["JBOLO_MODELS_PATH"], "V3r8/V3r8_Goal/LAT/V3r8_Goal_LAT_LF.yaml"
        ),
        "MF": os.path.join(
            os.environ["JBOLO_MODELS_PATH"], "V3r8/V3r8_Goal/LAT/V3r8_Goal_LAT_MF.yaml"
        ),
        "UHF": os.path.join(
            os.environ["JBOLO_MODELS_PATH"], "V3r8/V3r8_Goal/LAT/V3r8_Goal_LAT_UHF.yaml"
        ),
    },
}
...
def get_rand_offsets(r: float = 10.0, num: int = 50):
...
def get_aperture_phot(imap, cent, pixsize, r=2.4, r_rand=10, num_rand=50):
    ...
    for i, cur_cent in enumerate(cents):
        cur_stamp = imap[
            int(cur_cent[0]) - r : int(cur_cent[0]) + r,
            int(cur_cent[1]) - r : int(cur_cent[1]) + r,
        ]
...
def gauss(pixmap, mu_x, mu_y, amp, var):
    ...
    return np.array(
        amp * np.exp(-0.5 * ((xx - mu_x) ** 2 + (yy - mu_y) ** 2) / var)
    ).ravel()
...
def rad_avg(x_arr, y_arr, power, rmin, rmax, inc, shift_center=None):
    ...
    # calculate the mean
    def rad_bins(r_b):
        return (power)[(r_i >= r_b - (inc)) & (r_i < r_b + (inc))].mean()
...
def fit_gauss_pointing(imap, ivar, pixmap, make_plots=True):
    ...
    guess = [max_pix[1], max_pix[0], imap[max_pix[0], max_pix[1]], 10]
    ...
    data_solid_angle = solid_angle(
        x_centered_integrand, y_centered_integrand, normalize(imap)
    ) * (1e6)
    ...
    if make_plots:
        ...
        plt.plot(
            radii_data, normalize(means_data), label=f"Data: {data_solid_angle:.2f}"
        )
        plt.plot(radii_data, normalize(means_fit), label=f"Fit: {fit_solid_angle:.2f}")
        plt.title(r"beam solid angle [$\cdot 10^{-6}$ sr]")
        plt.legend()
        plt.ylim(1e-4, 1)
        plt.yscale("log")
        plt.xlabel("[arcmin]")
        ...
        return (
            ...
            shift_y,
            fwhm,
            np.max(np.array([data_solid_angle, fit_solid_angle])),
            chisred,
            popt,
            pcov,
            radii_data,
...
def get_pwv_obs(obs_id, pwv_data, n_hours_before, n_hours_after):
    ...
    msk = np.all(
        [
            pwv_data["timestamp"] > timestamp - n_hours_before * 3600,
            pwv_data["timestamp"] < timestamp + n_hours_after * 3600,
        ],
        axis=0,
    )
...
def get_inner_radial_mask(
    resid_map: enmap, pixsize: float, radius: float
) -> np.ndarray:
    """
...
def get_resid_rmse(resid_path: str, band: str) -> float:
    ...
    mask = get_inner_radial_mask(
        resid_map=resid_map,
        pixsize=np.abs(resid_map.wcs.wcs.cdelt[0]) * 60,
        radius=get_beam_keepout(band=band),
    )
