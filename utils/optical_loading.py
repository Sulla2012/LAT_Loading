...
def find_smurf_bgmap_file(bgmap_name: str, ot: str, ufm_name: str) -> str:
    ...
    try:
        ...
        bg_setup_filepath = (
            f"/so/data/{platform}/smurf/smurf_{bgmap_num}_{platform}/{ufm_name}"
        )
...
def load_ivs_from_times(
    ot: str,
    start: dt.datetime = dt.datetime(2025, 2, 20),
    end: dt.datetime = dt.datetime.now(),
    ufm_names: list[str] | None = None,
        ...
        for ufm_name in ufm_names:
            if ufm_name == obs_ufm_name:
                if full_load:
                    ...
                    ivs[ufm_name][obs["obs_id"]]["meta"]["tunefile"] = iv_data["meta"][
                        "tunefile"
                    ]
                    ivs[ufm_name][obs["obs_id"]]["meta"]["bgmap_file"] = iv_data[
                        "meta"
                    ]["bgmap_file"]
        ...
        for obs_id in list(ivs[ufm_name].keys()):
            if ivs[ufm_name][obs_id] is not None:
                ...
                try:
                    ...
                    rset = dm.ResSet.from_tunefile(
                        tunefile=tune_file,
                        bgmap_file=bgmap_file,
